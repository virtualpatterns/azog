...
task('count', [], { 'async': true }, () => {
  Jake.exec([ 'scripts/find-dirty-files' ], { 'printStderr': true, 'printStdout': true }, () => complete())
})
...
task('build', [ 'clean', 'count', 'lint' ], { 'async': true }, () => {
  Jake.exec([
    ...[ 'command', 'sandbox', 'tests' ].map((folderName) => `babel --config-file ./distributable/babel.configuration source/${folderName} --copy-files --out-dir distributable/${folderName} --source-maps`),
...
task('test', [ 'build' ], { 'async': true }, () => {
  Jake.exec([ 'mocha --bail --timeout 0 distributable/tests/index.js' ], { 'printStderr': true, 'printStdout': true }, () => complete())
})
